...
GLdouble posicaoCamera4X = 75;//75
...
//GLuint texturaMarteSuperficie;
//GLuint texturaMarteAtmosfera;
...
void desenharEsfera(double raio, double distancia, double inclinacaoX, double anguloRotacaoNoProprioEixo, double orbita, char *nome, bool verEsfera){
    ...
    /*if(verEsfera){
        ...
        double z = distancia * sin(((90 + orbita) * 3.141592) / 180);
        //double x = -sin(orbita * 180 * 3.1415) + distancia;
        double x = distancia * cos(((90 + orbita) * 3.141592) / 180);
    ...
    /*else if(strcmp(nome, "Marte") == 0){ //Tentei fazer marte ter uma atmosfera, mas a imagem da atmosfera que achei não tinha intensidade de 24 bits
        ...
        glColor3f(1.0, 1.0, 1.0);
        glEnable(GL_TEXTURE_2D);
        glBindTexture(GL_TEXTURE_2D, texturaMarteSuperficie);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        gluQuadricTexture(quadric, 1);
        gluSphere(quadric, raio, 20, 20);
        ...
        glBindTexture(GL_TEXTURE_2D, texturaMarteAtmosfera);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        gluQuadricTexture(quadric, 1);
        gluSphere(quadric, raio * 1.2, 20, 20);
        glDepthMask(GL_TRUE);
        glDisable(GL_BLEND);
        glDisable(GL_TEXTURE_2D);
    }*/
...
void Inicializa (void){
    ...
    //texturaMarteSuperficie = carregaTextura(superficieMarteTextura);
    //delete superficieMarteTextura;

    //Textura* atmosferaMarteTextura = carregarImagemBMP("Atmosfera.bmp");
    //texturaMarteAtmosfera = carregaTextura(atmosferaMarteTextura);
    //delete atmosferaMarteTextura;
...
void Animacao(int periodo){
    ...
    if(camera4Ligada == true && (posicaoCameraX != posicaoCamera4X || posicaoCameraY != posicaoCamera4Y || posicaoCameraZ != posicaoCamera4Z)){
        ...
        //Deslocamento Camera-Terra
        /*
        if(cameraTerra==true){
            ratioTour = (terra.raio/(sqrt(pow(terra.x-sol.x,2)+pow(terra.y-sol.y,2)+pow(terra.z-sol.z,2))));
            posicaoCameraX =(terra.x-sol.x)*ratioTour;
            posicaoCameraY =(terra.y-sol.y)*ratioTour;
            posicaoCameraZ = (terra.z-sol.z)*ratioTour;
        }
        */
   ...
   switch(key){
       ...
       case 's':
        if(comLuzDoSolOn && !iluminacaoEstaLigada){
            comLuzDoSolOn = false;
            printf("Luz do sol desligada\n");
        }
        else if(!comLuzDoSolOn && !iluminacaoEstaLigada){
            comLuzDoSolOn = true;
            printf("Luz do Sol ligada\n");
        }
        break;
